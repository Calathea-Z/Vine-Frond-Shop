...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const result = xml2js(xmlString, { compact: true, ignoreDeclaration: true });
      ...
      const suggestedAddress = valid ? null : {
      ...
      console.error(error);
      res.status(500).json({ error: "An error occurred while verifying the address." });
    }
  } else {
